...
import { studentAppointments } from "@/lib/dummyData";
            ...
            <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-[#5A6C7D] mt-1">
                        Review your past academic guidance sessions and interactions.
                    </p>
                </div>
            ...
            <div className="bg-white rounded-xl border border-[#DCE3ED] shadow-sm overflow-hidden">
                {studentAppointments.length > 0 ? (
                    <ul className="divide-y divide-[#DCE3ED]">
                        {studentAppointments.map((apt) => (
                            <li key={apt.id} className="p-5 hover:bg-[#F8FAFC] transition flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    <span
                                        className={`px-3 py-1 rounded-full text-xs font-semibold ${apt.status === 'Confirmed' ? 'bg-emerald-100 text-emerald-700' :
                        ...
                        {/* Adding an old realistic example payload */}
                        <li className="p-5 hover:bg-[#F8FAFC] transition flex flex-col sm:flex-row sm:items-center justify-between gap-4 opacity-75">
                            <div className="flex items-start gap-4 h-full">
                                <div className="bg-[#E8EEF5] rounded-lg p-3 text-center min-w-[60px] border border-[#DCE3ED]">
                                    <span className="block text-xs font-bold text-[#5A6C7D] uppercase">Sep 12</span>
                                    <span className="block text-lg font-bold text-[#1F3A5F]">2026</span>
                                </div>
                                <div className="flex flex-col h-full justify-between">
                                    <h4 className="font-semibold text-[#1F3A5F] text-lg">Dr. John von Neumann</h4>
                                    <p className="text-sm text-[#5A6C7D] flex items-center gap-2 mt-1">
                    ...
                    <div className="p-12 text-center text-[#5A6C7D]">
                        ...
                        <p className="text-lg font-medium text-[#1F3A5F]">No history found.</p>
                        <p className="mt-1">You haven't had any appointments yet.</p>
                    </div>
                )}
            </div>
        </div>
